...
<img src="readme.assets/image-20210816205709177.png" alt="image-20210816205709177" style="zoom:67%;" />
...
- **缘由：**那我们能不能不对比负例，只通过对比正例进行对比学习，这样不就可以规避上述问题了吗？(越大的batch size，就有越大的可能包含错误负例，由于infoNCE损失的特性，这些错误负例会极大地影响损失函数进而提供错误信号影响模型学习)
  - 这直观上是不现实的，因为如果不拉远与负例的距离
  - 只拉近正例间的距离
  - 那么模型很容易使所有样例的表示塌缩到同一个常数表示，这时损失为0，同时表示也失去了任何信息。
- **工作：**
  - 提出了一种形式上比较简单的方法来学习**图像的特征**表示
- **组成：**
